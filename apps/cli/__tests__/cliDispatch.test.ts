...
async function main() {
  ...
  assert(usageMessage().includes('yesno v1'), 'usage missing v1 command');
  ...
  const v1Mapped = await dispatchCommand('v1', ['--test-noexec']);
  assert(v1Mapped.kind === 'ok' && v1Mapped.command === 'v1', 'v1 dispatch mapping failed');
